	...
	for (int n = 0; n < columns_count; n++) {
	    ...
	    *column = ImGuiTableColumn();
		...
		true; // Preserve WidthAuto when reinitializing a live table:
		      // not technically necessary but remove a visible flicker
...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	min_column_width + table->CellPaddingX * 2.0f + table->CellSpacingX1 +
	table->CellSpacingX2;
    int count_fixed =
	...
	if (!column->IsPreserveWidthAuto) {
		...
		if (!(table->Flags & ImGuiTableFlags_Resizable) ||
		    ...
		    width_auto =
			ImMax(width_auto, column->InitStretchWeightOrWidth);
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		sum_width_fixed_requests +=
		    max_width_auto - column->WidthRequest; // Update old sum
	...
	if (table->Flags & ImGuiTableFlags_ScrollX) {
	    ...
	    // Because of this we also know that all of the columns will always
	    // fit in table->WorkRect and therefore in table->InnerRect (because
	    // ScrollX is off)
	    // FIXME-TABLE: This is solved incorrectly but also quite a
	    // difficult problem to fix as we also want ClipRect width to match.
	...
	column->WidthGiven = ImMax(
	    column->WidthGiven, ImMin(column->WidthRequest, min_column_width));
			     ...
			     float init_width_or_weight, ImGuiID user_id)
{
	...
	if (flags & ImGuiTableColumnFlags_DefaultSort &&
	    ...
	    column->SortOrder = 0; // Multiple columns using _DefaultSort will
...
void ImGui::TableMergeDrawChannels(ImGuiTable *table)
{
	...
	IM_ASSERT(has_freeze_v == false || table->Bg1DrawChannelUnfrozen !=
					       TABLE_DRAW_CHANNEL_BG1_FROZEN);
	int remaining_count =
	    splitter->_Count -
...
void ImGui::TableSortSpecsBuild(ImGuiTable *table)
{
    ...
    ImGuiTableColumnSortSpecs *sort_specs = single_sort_specs
...
void ImGui::TableHeader(const char *label)
{
	...
	if (pressed && table->ReorderColumn != column_n) {
	    ...
	    // - Note that the PreferSortAscending flag is never checked, it is
	    // essentially the default and therefore a no-op.
	    ImGuiSortDirection sort_direction;
	    if (column->SortOrder == -1)
		sort_direction =
		    (column->Flags & ImGuiTableColumnFlags_PreferSortDescending)
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	ImFormatString(
	    ...
	    column->DrawChannelUnfrozen, column->WidthGiven,
